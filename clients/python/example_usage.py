...
def main() -> None:
    ...
    for emb, response in embeddings:
        result = client.insert(embedding=emb, response=response, query_text="seed")
        print(f"Inserted id={result['id']}")

    hit = client.query(embedding=[1.0, 0.0, 0.0, 0.0], threshold=0.90)
    print(f"Exact match: hit={hit['hit']}, response={hit.get('response')}")

    miss = client.query(embedding=[0.5, 0.5, 0.5, 0.5], threshold=0.99)
